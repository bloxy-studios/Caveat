...
> **Artifacts are not committed to git** — they're regenerable. Run the train
> step first; it writes `models.joblib` + metrics/spec/samples into `artifacts/`.

```bash
cd ml
pip install -r requirements.txt

# 1) Train on the built-in synthetic dataset (no AMRFinderPlus / real data needed)
python -m caveat_ml.train --data synth --out artifacts

# 2) Score a held-out genome
python -m caveat_ml.predict --artifacts artifacts --list          # list held-out genome ids
python -m caveat_ml.predict --artifacts artifacts --genome <id>   # score one
```
...
Generated by `train` (not committed — regenerate anytime): `models.joblib`
(per-drug models), `feature_spec.json`, `metrics.json`, `meta.json`,
`sample_genomes.json` (held-out demo genomes).
